...
export async function getSearchResult(
  { messages, type }: { messages: CoreMessage[]; type?: 'default' | 'chat' },
  logger: WithLogger,
) {
  const openai = createOpenAI({
    apiKey: type === 'chat' ? CHAT_OPENAI_API_KEY : OPENAI_API_KEY,
  });
...
`,
  ...
  const { results } = await (type === 'chat' ? exaChat : exa).searchAndContents(
    text,
    {
      livecrawl: 'auto',
      numResults: 3,
